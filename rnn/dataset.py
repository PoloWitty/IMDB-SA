...
class Dataset(object):
    ...
    def get_train_test_dataloader(self,batch_size):
        ...
        # test_dataloader = DataLoader(small_test_dataset,shuffle=False,batch_size=batch_size)
...
if __name__=='__main__':
    ...
    tokenizer=Tokenizer.from_file('test/imdb/rnn/tokenizer.json')
    ...
    for post_batch in train_dataloader:
        ...
        print(post_batch["labels"].shape, post_batch["labels"])

        print()

        count[0] += len(post_batch["input_ids"])
        count[1] += post_batch["labels"].shape[0]
